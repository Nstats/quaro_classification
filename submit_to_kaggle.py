...
def model():
    ...
    with tf.variable_scope('BiRNN_encoder'):
        rnn_cell_fw = get_cell('gru', rnn_hidden_size, 'rnn_cell_fw',
                               rnn_layer_num, dp_keep_prob=dp_keep_prob_train, training=train_states)
    ...
    with tf.variable_scope('MLP'):
        ...
        layer1_d = tf.layers.dropout(layer1, 1 - dp_keep_prob_train, training=train_states)
        ...
        layer2_d = tf.layers.dropout(layer2, 1 - dp_keep_prob_train, training=train_states)
    ...
    with tf.Session() as sess:
        ...
        df_test, _ = preprocessing(test_dir, rectify_dict, train_or_dev=False)
        test_size = df_test.shape[0]
        prediction = []
        for i in range(int(1e10)):
            df = df_test.iloc[i * 100:(i + 1) * 100]
            x_test, x_valid_test, y_test = get_batch_data(df, int(df.shape[0]), embedding_index, False)
        ...
        print('write to submission successfully')
